...
export async function GET(req: NextRequest) {
  ...
  try {
    const url = `https://api.aniskip.com/v2/skip-times/${malId}/${episode}?types[]=op&types[]=ed&types[]=mixed-op&types[]=mixed-ed&episodeLength=0`;
    ...
    if (!json.found || !json.results) {
      // Don't cache misses — retry next time
      return NextResponse.json({ intro: null, outro: null });
    }
